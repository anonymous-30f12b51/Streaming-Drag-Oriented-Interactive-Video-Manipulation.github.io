...
(() => {
    ...
    const self_forcing_previous_rows = [
        {
            ...
            video_all: {
                src: 'static/videos/self-forcing-previous/6/block_0_6_7_21_80_slow.mp4',
        ...
        },
        {
            ...
            video_all: {
                src: 'static/videos/self-forcing-previous/23/block_0_6_7_21_80_slow.mp4',
                caption: 'Edited Video: 0 - 60 frames',
            },
        },
        {
            ...
            video_all: {
                src: 'static/videos/self-forcing-previous/64/block_0_6_7_21_80_slow.mp4',
                caption: 'Edited Video: 0 - 60 frames',
            },
        },
    ];
    ...
    const self_forcing_contradict_rows = [
        ...
        {
            text_0: {
                html: '... <span style="color: blue;">the vehicle stays tightly to the right side of the path</span> ...',
            },
            video_0: {
        ...
        {
            text_0: {
                html: '... <span style="color: blue;">his eyes are fixed on the left side of the scene</span> ...',
            },
            video_0: {
        ...
        {
            text_0: {
                html: '... <span style="color: blue;">she is looking to the right side of the scene</span> ...',
            },
            video_0: {
  ...
  `;
    ...
    function renderRow(
        ...
        /*
          ...
          Arguments:
            totalColumns: number | falsy (if falsy, infer from rowData length)
            rowData:
              - Either an array of item objects
              - Or an object with keys like video_0, img_1, text_0, video_all, etc.
          Item object conventions:
            type: 'video' | 'image' | 'text' (optional; inferred from key if absent)
            options: passed to imageCol / videoCol / textCol (e.g., { captionColor: '#ff0000' })
            For image/video:
              src, alt (image), caption, captionColor (optional)
            For text:
              text (escaped) OR html (trusted, not escaped), captionColor (optional)
          Ordering:
            Keys with numeric suffix (_0, _1, ...) sorted ascending.
            Non–numeric suffix keys appended after numeric ones preserving original order.
        */
        if (!rowData) return '';

        // Normalize to an array of items with a key field.
        let items = [];
        if (Array.isArray(rowData)) {
            items = rowData.map((item, idx) => {
                if (!item.key) item.key = `col_${idx}`;
                return item;
            });
        } else if (typeof rowData === 'object') {
            items = Object.entries(rowData).map(([key, val]) => {
                // If val is primitive string for text convenience
                if (typeof val === 'string') {
                    val = { text: val };
                }
                // Attach key so we can infer type/order
                return { key, ...val };
            });
        }
        ...
        // Removed numeric-suffix sorting to preserve user-specified order.

        const effectiveColumns = totalColumns || items.length || 1;
        const parts = [];
        ...
        items.forEach(item => {
            ...
            let type = item.type;
            if (!type && item.key) {
                if (item.key.startsWith('video')) type = 'video';
                else if (item.key.startsWith('image')) type = 'image';
                else if (item.key.startsWith('text')) type = 'text';
            }
            // Fallback inference.
            if (!type) {
                if (item.html || item.text) type = 'text';
                else if (item.src) {
                    // Prefer video if explicitly flagged
                    if (item.isVideo || item.key?.includes('video')) type = 'video';
                    else type = 'image';
                } else {
                    type = 'text';
                }
            ...
            const opts = item.options || {}; // item-level options override

            switch (type) {
                case 'video':
                    parts.push(videoCol(effectiveColumns, item, opts));
                    break;
                case 'image':
                    parts.push(imageCol(effectiveColumns, item, opts));
                    break;
                case 'text':
                    // Normalize text item shape for textCol
                    const textItem = {
                        text: item.text,
                        html: item.html,
                        captionColor: item.captionColor
                    };
                    parts.push(textCol(effectiveColumns, textItem, opts));
                    break;
                default:
    ...
    function renderGrid(
        ...
        if (!galleryId || !Array.isArray(data)) return;
        const mount = document.getElementById(galleryId);
        if (!mount) return;

        const headerHtml = (Array.isArray(headers) && headers.length)
            ? renderHeaderRow(totalColumns || headers.length, headers)
            : '';

        const rowsHtml = data.map(row => {
            const rowColumns = (row && (row.totalColumns ?? row.columns ?? row.colCount)) ?? totalColumns;
            const colsHtml = renderRow(rowColumns, row);
            return `<div class="columns is-multiline">${colsHtml}</div>`;
        }).join('\n');

        mount.innerHTML = headerHtml + rowsHtml;
    }


    function renderAllGalleries() {
        renderGrid(self_forcing_rows, 'self-forcing-gallery');
        renderGrid(causvid_rows, 'causvid-gallery');
        renderGrid(self_forcing_long_rows, 'self-forcing-long-gallery',
        ...
        renderGrid(self_forcing_failure_rows, 'self-forcing-failure-gallery');
    }


    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', renderAllGalleries);
    } else {
        renderAllGalleries();
    }
})();
